#!/usr/bin/env python
"""Run the real xkernels verify + verify_parity harness on a CUTE card, on ds5.

Reusable across the CUTE CUDA cards: ``ds5_verify_card.py <card-id> <op-id>``.
Defaults to the dual_rmsnorm card. This is the actual gate (meta/docs/library.md §5):
verify() checks the card vs the op's one reference across the mandatory shape
sweep; verify_parity() checks backends agree within cross_backend_rtol.
"""
...
CARD = sys.argv[1] if len(sys.argv) > 1 else "dual_rmsnorm.cuda@1.0.0"
OP = sys.argv[2] if len(sys.argv) > 2 else "dual_rmsnorm@1.0.0"
...
if "error" in r.get("artifacts", {}):
    print("  error:", str(r["artifacts"]["error"])[:400])
...
print(f"  errors                 = { {k: str(v)[:120] for k,v in p['errors'].items()} }")
