...
portable path (per `docs/library.md`).
...
end-to-end)**, identical numerics. Source: `scripts/ds5_c_solved_test.py`,
`scripts/ds5_jitcache_probe.py`. This pattern is duplicated verbatim in all 5
...
`scripts/ds5_c_final_probe.py`: `handle(gA, gB, gOut)` OK;
...
Confirmed by `scripts/ds5_dsl_math_probe2.py`, which grepped the installed
...
Confirmed working on sm_121 by `scripts/ds5_dsl_rowsum_probe.py`. The primitives
...
reference's `x.float()`. Proven by `scripts/ds5_bf16_load_probe.py`:
...
(`scripts/ds5_roofline_survey.py`), apply only where the op is memory-bound AND
...
`scripts/ds5_dsl_math_probe2.py` / `ds5_dsl_reduction_probe.py`): define a 4-element
...
Every probe script under `scripts/ds5_*.py` is kept as the reproducible trail —
...
python scripts/ds5_verify_card.py mm_fp8_blockscale.cuda@1.0.0 mm_fp8_blockscale@1.0.0
python scripts/ds5_cute_perf.py
# the API probes (re-derive any convention that moved):
python scripts/ds5_dsl_math_probe2.py        # math intrinsics
python scripts/ds5_dsl_rowsum_probe.py       # reduction primitives
python scripts/ds5_bf16_load_probe.py        # bf16-native-read
python scripts/ds5_roofline_survey.py        # which regime each card is in
```

Full testbed runbook (environment setup, the roofline survey, all the perf-pass
narratives): [`docs/ds5-cute-testbed.md`](../docs/ds5-cute-testbed.md).
