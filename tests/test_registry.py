...
@pytest.mark.parametrize("card_id", [
    ...
    # DSL-authored (vkl): plain single-tensor RMSNorm (issue #66). The body IS
    # the auto-reference; the @kernel decorator auto-wires REFERENCE dispatch +
    # the seeded input gen, so the reference card passes verify on CPU like any
    # hand op.
    "rmsnorm.reference@1.0.0",
    # DSL-authored (vkl): gated activations (issue #67, elementwise launch) and
