...
- **beverin**: `scripts/slurm/bench_all_beverin.sbatch` → `meta/benchmarks/bench_all.py`
  (9 ops) + `scripts/archive/issues/bench_fp8_blockscale_beverin.sbatch` →
  `meta/benchmarks/bench_fp8_blockscale_gemm.py` (the 10th, gfx942-only).
- **bristen**: `scripts/slurm/bench_all_bristen_isolated.sbatch` → a shell loop that
  runs `meta/benchmarks/bench_one.py <kernel>` **once per kernel in its own process**
...
One builder per op lives in `meta/benchmarks/probe_omniperf.py` (AMD) and
`meta/benchmarks/probe_ncu.py` (NVIDIA) — the twins share identical seeded shapes so a
...
- **beverin** (`scripts/slurm/profile_omniperf_beverin.sbatch` →
...
- **bristen** (`scripts/slurm/profile_ncu_bristen.sbatch` →
...
```bash
# bench (beverin)
scripts/cluster.sh submit --host beverin                              # 9-op table
...
rcc --profile bristen run -- env REPO=/capstor/scratch/cscs/xyao/xkernels \
    sbatch scripts/slurm/bench_all_bristen_isolated.sbatch
# profile (beverin)
KERNEL=dual_rmsnorm scripts/cluster.sh submit --host beverin scripts/slurm/profile_omniperf_beverin.sbatch
# profile (bristen)
