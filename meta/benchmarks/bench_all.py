...
The distributed `hierarchical_all_reduce` (see meta/docs/kernels/comm.md) is not a
...
Run on one gfx942 GPU (see scripts/slurm/bench_all_beverin.sbatch). Timing uses
`xkernels.utils.benchmarking.benchmark` (Triton `do_bench` when available).

Usage::

    python meta/benchmarks/bench_all.py
"""
...
def bench_ffn(dev):
    ...
    # this shape, see meta/benchmarks/probe_ffn.py). FFN is the only GEMM-bound op, so
